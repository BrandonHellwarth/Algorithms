...
class DoublyLinkedList {
  ...
  insertAfter(targetVal, newVal) {
    ...
    while(runner.data != targetVal && runner != null){
      runner = runner.next;
    }
    if(runner == null){
      return false;
      ...
      var currVal = newVal;
      var oldVal = runner.next.data;
      while(runner.next != null){
        runner.next.data = currVal;
        runner = runner.next;
        if(runner.next == null){
          runner.next = new DLLNode(oldVal);
          console.log(this.toArray());
          return true;
        }
        var oldVal2 = runner.next.data;
        runner.next.data = oldVal;
        oldVal = oldVal2;
        runner = runner.next;
        if(runner.next == null){
          runner.next = new DLLNode(oldVal);
